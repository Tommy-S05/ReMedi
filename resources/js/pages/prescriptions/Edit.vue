...
import type { BreadcrumbItem, PrescribedMedication, Prescription } from '@/types';
...
interface PrescribedMedicationFormDetail {
    ...
    id?: number; // ID del registro en la tabla pivote
    name?: string; // Para mostrar en el UI
 ...
 */
interface Props {
    prescription: Prescription; // La receta que se está editando
    userMedications: UserMedicationOption[]; // Lista de todos los medicamentos del usuario
}
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    },
    {
        ...
        href: route('prescriptions.edit', props.prescription.id),
    },
];

/**
 * Inicializa los detalles de los medicamentos para el formulario,
 * asegurando que el nombre del medicamento se incluya para la UI.
 * @param {PrescribedMedication[]} prescribedMeds - Medicamentos actualmente en la receta.
 * @returns {PrescribedMedicationFormDetail[]}
 */
const initializeMedicationDetails = (prescribedMeds: PrescribedMedication[]): PrescribedMedicationFormDetail[] => {
    return prescribedMeds.map((pm) => ({
        medication_id: pm.id, // El ID del modelo Medication
        name: pm.name, // Nombre del Medication para UI
        dosage_on_prescription: pm.pivot.dosage_on_prescription || '',
        quantity_prescribed: pm.pivot.quantity_prescribed || '',
        instructions_on_prescription: pm.pivot.instructions_on_prescription || '',
    }));
};

const form = useForm<PrescriptionForm>({
    title: props.prescription.title || '',
    doctor_name: props.prescription.doctor_name || '',
    prescription_date: props.prescription.prescription_date || new Date().toISOString().split('T')[0],
    notes: props.prescription.notes || '',
    medication_details: initializeMedicationDetails(props.prescription.medications),
});
...
const submit = () => {
    ...
    form.transform(() => dataToSubmit).put(route('prescriptions.update', props.prescription.id), {
        onSuccess: () => {
            toast.success(t('Prescription updated successfully!'), {
                description: t('The prescription :title has been updated successfully.', { title: form.title || '' }),
                duration: 5000,
            });
            form.reset();
        },
...
<template>
    <Head :title="t('Edit Prescription') + (props.prescription.title ? ` - ${props.prescription.title}` : ` #${props.prescription.id}`)" />
            ...
            <div class="mx-auto w-full max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
                <div class="xs:flex-row xs:items-center xs:justify-between mb-8 flex w-full flex-col gap-4">
                    <h1 class="text-primary dark:text-remedi-light-blue flex items-center text-2xl leading-tight font-semibold">
                        ...
                        {{ t('Edit Prescription') }}
                        <span v-if="props.prescription.title" class="text-muted-foreground ml-2 text-xl font-normal"> - {{ props.prescription.title }}</span>
                        <span v-else class="text-muted-foreground ml-2 text-xl font-normal"> #{{ props.prescription.id }}</span>
                    </h1>
